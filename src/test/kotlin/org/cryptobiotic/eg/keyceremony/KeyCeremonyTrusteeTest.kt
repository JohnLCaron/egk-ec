...
class KeyCeremonyTrusteeTest {
    ...
    @Test
    fun testPublicKeys() {
        groups.forEach { testPublicKeys(it) }
    }

    fun testPublicKeys(group: GroupContext) {
    ...
    fun testReceivePublicKeysBadQuorum() {
        groups.forEach { testReceivePublicKeysBadQuorum(it) }
    }

    fun testReceivePublicKeysBadQuorum(group: GroupContext) {
    ...
    fun testReceivePublicKeysBadReceiver() {
        groups.forEach { testReceivePublicKeysBadReceiver(it) }
    }

    fun testReceivePublicKeysBadReceiver(group: GroupContext) {
        val trustee1 = KeyCeremonyTrustee(group, "id1", 41, 5, 4)
    ...
    fun testReceivePublicKeysBadProofs() {
        groups.forEach { testReceivePublicKeysBadProofs(it) }
    }

    fun testReceivePublicKeysBadProofs(group: GroupContext) {
        val trustee1 = KeyCeremonyTrustee(group, "id1", 41, 5, 4)
        ...
        assertContains(
            result1.error,
            "  Guardian bad has invalid proof for coefficient 0 inBoundsU=true validChallenge=false"
        )
    }

    @Test
    fun testShareEncryptDecrypt() {
        groups.forEach { testShareEncryptDecrypt(it) }
    }

    fun testShareEncryptDecrypt(group: GroupContext) {
        val trustee1 = KeyCeremonyTrustee(group, "id1", 41, 5, 4)
    ...
    fun testReceiveSecretKeyShare() {
        groups.forEach { testReceiveSecretKeyShare(it) }
    }

    fun testReceiveSecretKeyShare(group: GroupContext) {
        val trustee1 = KeyCeremonyTrustee(group, "id1", 41, 5, 4)
        val trustee2 = KeyCeremonyTrustee(group, "id2", 42, 5, 4)
        ...
        assertEquals(
            "ReceiveEncryptedKeyShare 'id1' sent share to wrong trustee 'id1', should be availableGuardianId 'id2'",
            result5.error
        )
    }
    ...
    fun testValidateSecretKeyShare() {
        groups.forEach { testValidateSecretKeyShare(it) }
    }

    fun testValidateSecretKeyShare(group: GroupContext) {
        val trustee1 = KeyCeremonyTrustee(group, "id1", 41, 5, 4)
        val trustee2 = KeyCeremonyTrustee(group, "id2", 42, 5, 4)
    ...
    @Test
    fun testKeyShareFor() {
        groups.forEach { testKeyShareFor(it) }
    }

    fun testKeyShareFor(group: GroupContext) {
        val trustee1 = KeyCeremonyTrustee(group, "id1", 41, 5, 4)
        val trustee2 = KeyCeremonyTrustee(group, "id2", 42, 5, 4)
        ...
        assertEquals(
            "Trustee 'id1', does not have KeyShare for 'bad'; must call encryptedKeyShareFor() first",
            result6.error
        )
    }
    ...
    fun testReceiveKeyShare() {
        groups.forEach { testReceiveKeyShare(it) }
    }

    fun testReceiveKeyShare(group: GroupContext) {
        val trustee1 = KeyCeremonyTrustee(group, "id1", 41, 5, 4)
        val trustee2 = KeyCeremonyTrustee(group, "id2", 42, 5, 4)
        ...
        assertEquals(
            "Sent KeyShare to wrong trustee 'id1', should be availableGuardianId 'id2'",
            resultWrongTrustee.error
        )
